...
##20 KISS 原则 和 YAGNI 原则：

    KISS 原则： 尽量保持简单

        KISS 原则就是保持代码可读和可维护的重要手段。代码足够简单、意味者很容易读懂，bug比较难隐藏，既然有，修改也更简单。
        
    YAGNI 原则：不要做过度设计

        不要去设计当前用不到的功能；不要去编写当前用不到的代码。
    
## 21 如何提供代码的复用性，DRY 原则：

    DAY 原则：Don't Repeat Yourself 不要写重复的代码.

    三种代码重复情况：实现逻辑重复、功能语义重复和代码执行重复。

    实现逻辑重复：尽管代码实现逻辑相同，但语义不同，并不违反 DRY 原则：
    
    功能语义重复：尽管两段代码的实现逻辑不重复，但语义重复，也就是功能重复，我们认为它违反了 DRY 原则。

    代码执行重复：既没有逻辑重复，也没有语义重复，代码执行重复，仍然违反来 DRY 原则。
