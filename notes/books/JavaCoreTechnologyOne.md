...
## 第四章 对象与类
    ...
    4.1 面向对象程序设计概述：
        面向对象与面向过程区别：
        面向过程：传统的结构化程序设计一些列的过程(即算法)来解决问题。
        一旦确定来过程，就开始考虑存储数据的方式。程序 = 算法 + 数据。算法第一位，数据第二位。
        面对对象：换个次序，将数据防第一位，然后在考虑操作数据的算法。在OOP中不必关心对象的具体实现，只要能满足需求即可。
        面向对象跟个适用于解决规模较大的问题。


        对象：想要使用OOP,一定要清楚对象的三个主要特征：
            对象的行为(behavior)--可以对对象施加那些操作，或可以对对象施加那些方法。
            对象的状态(state)--当施加那些方法时，对象如果响应。对象的状态的改变必须通过调用方法实现。
            对象标识(identity) -- 如何辨别具有相同行为与状态的不同对象。对象的唯一身份，每个对象用不相同。

            并不是所有类都具体面向对象都特征。例如 Math类。
        
        类之间的关系：
            依赖： users-a : 如果一个类的方法操纵另一个类对象，我们说一个类依赖另一个类。
            聚合： has-a : 类 A 的对象包含类 B 的对象。
            继承： is-a ： 特殊到一般的关系

        final实例域:
            实例域定义为 final 构建对象必须初始化，构建之后这个值不可修改。
            对于可变类，final 关键字表示存储变量中的引用不会再指向其他对象。
            
        静态域 static:
            如果将域定义为 static,每个类中执偶一个这样的域，
            而每个对象对于所有的实例域都有自己的一份拷贝。
            静态域，属于类，而不属于任何独立的对象。    
        
        静态方法:
            静态方法是一种不能想对象实施操作的方法。换句话说，没有隐式的参数。
            
            哪些情况下使用静态方法:
                一个方法不需要访问对象状态,其所需参数都是通过显示参数提供。
                一个方法只需要访问类的静态域。
                静态方法另一个用途使用静态工厂方法来构造对象。
        方法参数：
            一个方法不能修改一个基本数据类型都参数(即数值型或布尔型)
            一个方法可以改变一个对象参数的状态。
            一个方法不能让对象的参数引用一个新的对象。（方法是引用的拷贝）。
...
## 第五章 继承
    ...
    super与this：
        ...
        super 不是一个对象的引用，不能将 super 赋值给另一个对象变量。
        它只是一个指示编译器调用超类方法的特殊关键字。使用super调用超类的方法。
        this 是一个引用。
        this有两个用途：1、引用隐式参数 2、调用该类其他的构造器。
        super关键字两个用途：1、调用超类的方法 2、调用超类的构造器。

        
        在子类种可以添加域、增加方法或覆盖超类的方法，但绝不能删除继承但任何域和方法。
    ...
    子类构造器：
        ...
        super(name, salary);调用超类Employee 中含有name, salary参数构造器。
        由于 Manager类但构造器不能访问 Employee类的私有域，所以必须利用 Employee类
        的构造器对这部分私有域进行初始化，我们可以通过 super 实现对超类构造器对调用。
        使用super调用构造器对语句必须是子类构造器对第一个语句。

        如果子类对构造器灭蝇显示的调用超类的构造器，则将自动地调用超类默认的构造器。
        如果超类没有无参构造器，并且子类构造器中没有显示地调用超类其他构造器，则报错。
    
    多态：
        继承就是 is-a 规则，另一种表述法是置换法则，表明程序中出现超类对象的任何地方都可以用子类对象置换。
        例如：可以将一个子类都对象赋给超类变量
            Employee e; 
            e = new Employee();
            e = new Manager();
    ...
    阻止继承：final类和方法：
        定义类的时候使用了 final 修饰法，表明这个类是 final 类。
        public final class Exective extends Manager{...}

        类中特定的方法也可以被声明 final，如果这样做，子类就不能覆盖这个方法。
    ...
    强制类型转换：
        在Java中，每个对象变量都属于一个类型。
        只能在继承层次内进行类型转换。
        在将超类转换成子类之前，应该使用 instanceof 进行检查。

    抽象类：
        抽象类不能被实例化，也就是说，如果将一个类声明为abstract,就不能创建这个类都对象。
